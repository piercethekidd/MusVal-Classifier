...
def run():
	...
	cv = TfidfVectorizer(min_df=1, stop_words='english', lowercase=True, tokenizer=LemmaTokenizer())
	...
	######
	scores = cross_val_score(clf, cv.transform(data_x), data_y['valence'], cv=KFold(n_splits=10, shuffle=True))
